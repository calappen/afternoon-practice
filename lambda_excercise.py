...
prog_lang_version = sorted(prog_lang, key = lambda x: x[1], reverse=False)
...
prog_lang_name = sorted(prog_lang, key = lambda x: len(x[0]), reverse=True)
print(prog_lang_name)

#3. Filter the list so that it only contains languages with 'a' in it.
#[('Java', 13), ('JavaScript', 2019), ('Scala', 2.13)]

prog_lang_filter_a = list(filter(lambda x: ('a' in x[0]), prog_lang))
prog_lang_filter_a.sort(key = lambda x: x[0])
print(prog_lang_filter_a)

#4. Filter the list so that it only contains languages whose version is in integer form.
#[('Java', 13), ('JavaScript', 2019)]

prog_lang_filter_v = list(filter(lambda x: (isinstance(x[1],int)), prog_lang))
print(prog_lang_filter_v)
...
prog_lang_lower = list(map(lambda x: (x[0].lower(),len(x[0])), prog_lang))
...
#prog_lang_tuple = ((list(map(lambda x: (x[0]), prog_lang))),list((map(lambda x: (x[1]), prog_lang))))                            

prog_lang_tuple = (','.join(map(lambda x: (x[0]),prog_lang)),','.join(map(lambda x: str((x[1])),prog_lang)))
